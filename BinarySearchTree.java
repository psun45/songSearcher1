...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
      ...
      if (((parent.down[0] != child) && (parent.down[1] != child)) || (parent.down[0] == null && parent.down[1] == null)) {
        throw new IllegalArgumentException("not related - problem");
      }
      
      BinarySearchTree.Node<T> grandma = parent.up;         // parent of the parent, needs to be reassigned after rotation
      ...
      if (parent.down[0] == child) {
        ...
        parent.down[0] = child.down[1];   // parent's left child becomes the child's right child, no longer the child
        
        if (child.down[1] != null) { 
          child.down[1].up = parent;      // if possible, assign child's right child up to the parent
        }
        
        child.down[1] = parent;           // move child up and parent down to right of child
        parent.up = child;    
        ...
        if (grandma != null) {
          if (grandma.down[0] == parent) {  // if parent was a left child of grandma, assign grandma's left to child
            grandma.down[0] = child;
          }
          else {                            // if parent was a right child of grandma, assign grandma's right to child
        ...
        parent.down[1] = child.down[0];   // parent's right child becomes the child's left child, no longer the child
        
        if (child.down[0] != null) { 
          child.down[0].up = parent;      // if possible, assign child's left child up to the parent
        }
        
        child.down[0] = parent;           // move child up and parent down to left of child
        parent.up = child;    
        ...
        if (grandma != null) {
          if (grandma.down[1] == parent) {  // if parent was a left child of grandma, assign grandma's left to child
            grandma.down[1] = child;
          }
          else {                            // if parent was a right child of grandma, assign grandma's right to child
            grandma.down[0] = child;
	...
	// Tests the right rotate method on a size 7 tree at the top level
      ...
      Node<String> red = test.root;
      Node<String> blue = red.down[0];
      Node<String> yellow = red.down[1];
      Node<String> green = blue.down[0];
      Node<String> purple = blue.down[1];
      Node<String> orange = yellow.down[0];
      Node<String> pink = yellow.down[1];
      
      test.rotate(blue, red);
      
      if (test.root != blue) {return false;}    // check to make sure root was updated
      if (blue.down[1] != red) {return false;}  // checks to make sure the red and blue switched
      if (blue.down[0] != green) {return false;}    // checks to make sure green stayed with blue
      if (red.down[0] != purple) {return false;}    // red has new Left child and it is purple
  ...
  // Tests the left rotate method on a size 7 tree at a mid level
      ...
      test.rotate(purple, blue);
      
      if (test.root != red) {return false;}     // check to make sure root stayed the same
      if (blue.down[0] != green) {return false;}    // checks to make sure green stayed with blue
      if (red.down[0] != purple) {return false;}    // red has new Left child and it is purple
      if (red.down[1] != yellow) {return false;}    // red has same right child and it is yellow
    ...
    public static boolean test3() {
        // TODO: Implement this test.
        // attempt to rotate two non linked nodes
      ...
      try {
        test.rotate(red, yellow);
        return false;
      }
      
      catch (IllegalArgumentException e) {
        // should not catch a exception
        return true;
      }
          }
